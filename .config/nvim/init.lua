...
vim.diagnostic.config {
  ...
  virtual_text = {
    ...
    format = function(diagnostic)
      if #diagnostic.message > 40 then
        return '' -- hide inline; virtual_lines will show it below
      end
  ...
  virtual_lines = {
    ...
    format = function(diagnostic)
      if #diagnostic.message <= 40 then
        return '' -- short ones already shown inline, skip
      end
      return diagnostic.message
    end,
  },
...
vim.api.nvim_create_autocmd('InsertLeave', {
  callback = function()
    if vim.bo.modified and vim.bo.buftype == '' then
      if autosave_timer then autosave_timer:stop() end
      autosave_timer = vim.defer_fn(function()
        if vim.bo.modified and vim.bo.buftype == '' then
          require('conform').format { async = false }
          vim.cmd 'write'
          vim.notify('AutoSave: saved ' .. vim.fn.expand '%:t', vim.log.levels.INFO)
        end
      end, 500) -- 500ms delay, adjust as needed
    end
  end,
})
...
require('lazy').setup({
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        vim.notify('format_on_save called: ' .. vim.bo[bufnr].filetype)
  ...
  { -- Multi-cursor support (VS Code's Cmd+D behavior)
    ...
    config = function()
      ...
      set({ 'n', 'v' }, '<C-d>', function() mc.matchAddCursor(1) end, { desc = 'Add cursor at next match' })
      set({ 'n', 'v' }, '<C-S-d>', function() mc.matchSkipCursor(1) end, { desc = 'Skip match, add cursor at next' })

      -- Add cursor above/below
      set({ 'n', 'v' }, '<C-Up>', function() mc.lineAddCursor(-1) end, { desc = 'Add cursor above' })
      set({ 'n', 'v' }, '<C-Down>', function() mc.lineAddCursor(1) end, { desc = 'Add cursor below' })

      -- Select all matches at once
      set({ 'n', 'v' }, '<leader>A', mc.matchAllAddCursors, { desc = 'Add cursors to all matches' })

      -- Toggle cursors on/off (if they got disabled)
      set({ 'n', 'v' }, '<leader>mt', mc.toggleCursor, { desc = '[M]ulticursor [T]oggle' })

      -- Explicit exit: clear all extra cursors
      set({ 'n', 'v' }, '<leader>me', mc.clearCursors, { desc = '[M]ulticursor [E]xit' })

      -- Smart <Esc>: clear cursors if any, else nohlsearch
      set('n', '<Esc>', function()
      ...
      end, { desc = 'Clear multicursors or search highlight' })
...
6. Do NOT repeat or copy any existing code around <cursorPosition>.
  ...
  { -- Shows current code context (function/class you're inside) at the top
