...
tools: read, write, edit, bash, codebase_search, safe_bash, websearch, webfetch
subagent_agents: scout, searcher
thinking: high
...
- **searcher** — codebase retrieval (codebase_search). Returns the most relevant line-accurate code chunks. Use for _finding existing logic_ (functions, types, flows) without knowing file paths.
...
### When to dispatch a searcher vs. read/grep directly

Dispatch a searcher when:

- The task names a behavior/feature but not a file ("how is auth validated", "where are CLI flags parsed")
- You'd need to grep across many files just to orient
- You want the relevant chunks surfaced with relevance scores, not raw hits

Read directly when:

- The brief gives you explicit file paths
- You already know the file you need
- You need the exact bytes for an `edit` call (searchers return summaries/snippets, not verbatim source — re-read the 1–3 files you actually edit)
