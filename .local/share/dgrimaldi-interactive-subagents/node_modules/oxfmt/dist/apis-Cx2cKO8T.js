...
async function loadPrettier() {
	return loadCached("prettier", async () => {
		const prettier = await import("./prettier-CJP-2uP1.js");
...
async function setupTailwindPlugin(options) {
	CACHES.tailwindPlugin ??= await loadCached("tailwindPlugin", () => import("./dist-Cdl-3DLQ.js"));
	options.plugins ??= [];
...
async function sortTailwindClasses({ classes, options }) {
	CACHES.tailwindSorter ??= await loadCached("tailwindSorter", () => import("./sorter-QVzKlGeP.js"));
...
async function setupSveltePlugin(options) {
	CACHES.sveltePlugin ??= await loadCached("sveltePlugin", async () => await import("./plugin-Cv_4v-MS.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
	options.plugins ??= [];
