...
| `model`   | string | the main thread's model | Model for this spawn. By default a sub-agent inherits the main thread's current model (falling back to the agent's frontmatter model only when the main thread has no model set). Set this to force a specific model. |
...
| ------------ | ------------------------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **scout**    | `openrouter/z-ai/glm-5.3` | `codebase_search`, `read`, `grep`, `find`, `ls`                                                     | Fast read-only codebase recon                         |
| **searcher** | `openrouter/z-ai/glm-5.3` | `codebase_search`, `safe_bash`                                                                      | Retrieves the most relevant code chunks via BM25      |
| **worker**   | `openrouter/z-ai/glm-5.3` | `read`, `write`, `edit`, `bash`, `codebase_search`, `safe_bash`, `websearch`, `webfetch` + spawning | General implementer; may spawn `scout` and `searcher` |

All three are autonomous (`auto-exit: true`) and carry their identity in the system prompt (`system-prompt: append`).

### Codebase search

`codebase_search` runs a dependency-free BM25 search over the working codebase and returns line-accurate chunks with relevance scores. It lets an agent pull only the relevant code into context instead of reading whole files — which matters most for small local models with a tight context window. The index is cached under `.memory/codebase-index/` (gitignored) and re-indexed incrementally as files change.

See **[RETRIEVAL.md](./RETRIEVAL.md)** for full usage: writing good queries, the `retriever.ts` API, tuning, and how indexing works.
...
thinking: medium
tools: read, edit, write, safe_bash, codebase_search
...
| `model`                    | string  | Fallback model. A spawned sub-agent normally inherits the main thread's current model; this frontmatter value is used only when the main thread has no model set (or when `subagent({ model })` is omitted).                                                                                                                                                                             |
| `thinking`                 | string  | `minimal`, `low`, `medium`, or `high`                                                                                                                                                                                                                                                                                                                                                    |
| `tools`                    | string  | Strict tool allowlist. Built-ins: `read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`. Extension-backed: `codebase_search`, `safe_bash`, `websearch`, `webfetch`. Only the extensions backing the listed tools are loaded into the child. `websearch`/`webfetch` come from the globally-installed web-tools extension (`~/.local/share/dgrimaldi-web-search/extensions/src/index.ts`). |
