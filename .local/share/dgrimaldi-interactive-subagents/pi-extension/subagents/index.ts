...
function getToolExtensionPath(tool: string): string | undefined {
  ...
  // Local-only tool extensions plus the globally-installed web-tools extension
  // (web_search/web_fetch). The remote tools that used to live here
  // (video_extract, youtube_search, google_image_search) are intentionally
  // omitted.
  const map: Record<string, string> = {
    safe_bash: join(SUBAGENTS_DIR, "tools", "safe-bash.ts"),
    codebase_search: join(SUBAGENTS_DIR, "tools", "codebase-search.ts"),
    // Globally-installed web-tools extension (dgrimaldi-web-search). Its entry
    // is extensions/src/index.ts, which registers the `websearch` and
    // `webfetch` tools. Only loaded when an agent explicitly lists one of these
    // in its tools frontmatter.
    websearch: join(homedir(), ".local/share/dgrimaldi-web-search/extensions/src/index.ts"),
    webfetch: join(homedir(), ".local/share/dgrimaldi-web-search/extensions/src/index.ts"),
  };
...
/**
 * Resolve which model a spawned sub-agent should run.
 *
 * Precedence:
 *   1. Explicit per-call `model` override (highest — caller asked for it).
 *   2. The spawning (main-thread) session's current model — sub-agents inherit
 *      the same language model as the main thread.
 *   3. The agent's frontmatter `model` field.
 *
 * Returns `null` when none is available (the child then uses pi's default).
 */
function resolveSubagentModel(
  ...
  return paramsModel ?? parentModel ?? agentModel ?? null;
}
...
async function launchSubagent(
  ...
  ctx: {
    ...
    cwd: string;
    /** Current model id of the spawning (main-thread) session, if any. */
    model?: string;
  },
  ...
  const effectiveModel = resolveSubagentModel(params.model, ctx.model, agentDefs?.model);
...
export default function subagentsExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      let running;
      try {
        running = await launchSubagent(params, { ...ctx, model: ctx.model?.id });
      } finally {
