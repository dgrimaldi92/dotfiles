...
import * as subagentsModule from "../pi-extension/subagents/index";
...
} from "../pi-extension/subagents/session";

import { shellEscape } from "../pi-extension/subagents/tmux";
import {
  advanceStatusState,
  capStatusLines,
  classifyStatus,
  ...
  loadStatusConfig,
  parseStatusConfig,
} from "../pi-extension/subagents/status";
import {
  createSubagentActivityRecorder,
  getSubagentActivityFile,
  readSubagentActivityFile,
} from "../pi-extension/subagents/activity";
import {
  shouldMarkUserTookOver,
  shouldAutoExitOnAgentEnd,
  findLatestAssistantError,
  runningChildrenCount,
} from "../pi-extension/subagents/subagent-done";
import subagentDoneExtension from "../pi-extension/subagents/subagent-done";
import { __pollForExitTest__ } from "../pi-extension/subagents/tmux";
...
describe("session.ts", () => {
  ...
  describe("subagent loadout snapshot", () => {
    const sample: SubagentLoadout = {
      agent: "worker",
      toolAllowlist: "read,write,edit,safe_bash,codebase_search,subagent,ask_question",
      model: "openrouter/z-ai/glm-5.2",
      thinking: "medium",
      systemPromptMode: "append",
      identity: "You are a worker agent.",
      spawnable: ["scout", "searcher"],
      autoExit: true,
      cwd: "/work/dir",
...
describe("subagent discovery", () => {
  ...
  it("bundled scout/searcher/worker all resolve as non-interactive (auto-exit)", () => {
    for (const name of ["scout", "searcher", "worker"]) {
  ...
  it("worker is granted the spawning toolset restricted to scout and searcher", () => {
    const worker = testApi.loadAgentDefaults("worker");
    assert.ok(worker, "expected bundled worker to be discoverable");
    assert.deepEqual(worker.subagentAgents, ["scout", "searcher"]);
  ...
  it("resolveSubagentModel inherits the parent model but an explicit override wins", () => {
    // Explicit per-call override beats everything.
    assert.equal(
      testApi.resolveSubagentModel("explicit/model", "parent/model", "agent/model"),
      "explicit/model",
    );
    // No override → inherit the spawning (main-thread) session's model,
    // even over the agent's own frontmatter model.
    assert.equal(
      testApi.resolveSubagentModel(undefined, "parent/model", "agent/model"),
      "parent/model",
    );
    // No parent model (e.g. bare spawn) → fall back to the agent frontmatter.
    assert.equal(testApi.resolveSubagentModel(undefined, undefined, "agent/model"), "agent/model");
    // Nothing available → null (child uses pi's default).
    assert.equal(testApi.resolveSubagentModel(undefined, undefined, undefined), null);
  });

  it("scout and searcher are not granted spawning tools", () => {
    for (const name of ["scout", "searcher"]) {
      const defs = testApi.loadAgentDefaults(name);
      assert.ok(defs, `expected bundled agent ${name} to be discoverable`);
  ...
  it("getToolExtensionPath maps custom tools and skips built-ins", () => {
    ...
    assert.ok(
      testApi.getToolExtensionPath("codebase_search")?.endsWith("tools/codebase-search.ts"),
    );
    assert.ok(testApi.getToolExtensionPath("safe_bash")?.endsWith("tools/safe-bash.ts"));
    // Globally-installed web-tools extension.
    assert.ok(testApi.getToolExtensionPath("websearch")?.endsWith("extensions/src/index.ts"));
    assert.ok(testApi.getToolExtensionPath("webfetch")?.endsWith("extensions/src/index.ts"));
