import { parsePublicHttpUrl } from "../../shared/url-parser";
import { NormalizedSearchResult } from "../domain/types";

/** Parse a raw Tavily JSON search response and return just the `results` array as a string. */
export interface ParseTavilyResponseResult {
  readonly results: readonly NormalizedSearchResult[];
  readonly searchText: string;
}

export type ResultsParseError =
  | { readonly _tag: "NoRecognizedResults" }
  | { readonly _tag: "MalformedResultSection"; readonly reason: string };

export interface ParseSearchTextResult {
  readonly results: readonly NormalizedSearchResult[];
  readonly discardedSections: number;
  readonly explicitNoResults: boolean;
}

/** Parse an untrusted text search-result format into normalized results. */
export function parseSearchText(input: string): ParseSearchTextResult {
...
function parseSearchSection(section: string): NormalizedSearchResult | undefined {
  ...
  for (const line of lines) {
    if (!inText && line.toLowerCase().startsWith("title: ")) {
      title = line.toLowerCase().slice("title: ".length).trim();
      continue;
    }
    if (!inText && line.toLowerCase().startsWith("url: ")) {
      url = line.toLowerCase().slice("url: ".length).trim();
      continue;
    }
    ...
    if (!inText && line.startsWith("content: ")) {
      inText = true;
      snippetLines.push(line.slice("content:".length).trim());
      continue;
    }
    if (inText) {
