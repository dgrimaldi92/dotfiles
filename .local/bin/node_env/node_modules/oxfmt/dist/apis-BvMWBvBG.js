...
async function loadPrettier() {
	return loadCached("prettier", async () => {
		const prettier = await import("./prettier-DQwE6UFs.js");
...
async function setupTailwindPlugin(options) {
	CACHES.tailwindPlugin ??= await loadCached("tailwindPlugin", () => import("./dist-Fg3QNviF.js"));
	options.plugins ??= [];
...
async function sortTailwindClasses({ classes, options }) {
	CACHES.tailwindSorter ??= await loadCached("tailwindSorter", () => import("./sorter-BnxQHDq1.js"));
...
async function setupSveltePlugin(options) {
	CACHES.sveltePlugin ??= await loadCached("sveltePlugin", async () => await import("./plugin-D0HeBk_g.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
	options.plugins ??= [];
...
async function setupOxfmtPlugin(options) {
	CACHES.oxfmtPlugin ??= await loadCached("oxfmtPlugin", async () => await import("./prettier-plugin-oxfmt-ChVsPDso.js"));
	options.plugins ??= [];
